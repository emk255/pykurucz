...
from atlas_py.tools.validate_phase1 import _ensure_gfpred_assembled
...
def _run_fortran_atlas(
    *,
    ...
    iter_tag = f"iter{max(1, int(iterations))}"
    output_atm = out_fortran_dir / f"fortran_{iter_tag}.atm"
    ...
    cmd = [
        ...
        "--iterations", str(max(1, int(iterations))),
    ]
...
def _run_case(
    ...
    *,
    ...
    atlas_convergence_epsilon: float | None,
    atlas_convergence_min_iterations: int,
    atlas_convergence_consecutive: int,
    wipe: bool,
    ...
    if mode in ("fortran", "both"):
        iter_tag = f"iter{max(1, int(atlas_iterations))}"
        fortran_atm = out_fortran_dir / f"fortran_{iter_tag}.atm"
    ...
    if mode in ("python", "both"):
        iter_tag = f"iter{max(1, int(atlas_iterations))}"
        python_atm = out_python_dir / f"python_{iter_tag}.atm"
        ...
        python_npz = out_python_dir / f"python_{iter_tag}_synthe.npz"
...
""",
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Internal ATLAS iterations for both Fortran atlas12.exe and atlas_py (default: 1)",
    )
    p.add_argument(
        "--atlas-convergence-epsilon",
        type=float,
        default=None,
        help="Optional atlas_py physical-column early-stop epsilon; Fortran still runs --atlas-iterations",
    )
    p.add_argument(
        "--atlas-convergence-min-iterations",
        type=int,
        default=5,
        help="Minimum Python ATLAS iterations before convergence stop can trigger (default: 5)",
    )
    p.add_argument(
        "--atlas-convergence-consecutive",
        type=int,
        default=1,
        help="Consecutive converged Python ATLAS iterations required (default: 1)",
    )
...
def main() -> int:
    ...
    common_kwargs: dict = dict(
        ...
        atlas_iterations=max(1, args.atlas_iterations),
        atlas_convergence_epsilon=args.atlas_convergence_epsilon,
        atlas_convergence_min_iterations=max(1, args.atlas_convergence_min_iterations),
        atlas_convergence_consecutive=max(1, args.atlas_convergence_consecutive),
        wipe=args.wipe,
