...
def discover_default_molecular_line_directories() -> List[Path]:
    ...
    The ``data/`` directory is populated by running
    ``scripts/download_data.py`` once.  If the directory is absent or empty,
    returns an empty list (atomic-line synthesis only).
    """
